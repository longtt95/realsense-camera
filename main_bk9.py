...
def train_model():
    ...
    if feature_var.get() == 'HOG' and classification_var.get() == 'CNN':
        ...
        # Reshape the inputs to match the expected input structure
        X_train_2d = X_train_2d.reshape(-1, 64, 64, 1)
        X_train_3d = X_train_3d.reshape(-1, 64, 64, 64, 1)

        # Construct and train the model
        model = construct_combined_model((64, 64, 1), (64, 64, 64, 1), len(label_to_int))
    ...
    elif feature_var.get() == 'HOG' and classification_var.get() == 'CNN':
        # Load datasets
        train_images_2d, train_labels = load_2d_dataset('TrainData/2D/')
        train_images_3d, _ = load_3d_dataset('TrainData/3D/')
    ...
    elif feature_var.get() == 'Hu' and classification_var.get() == 'CNN':
        # Load datasets
        train_images_2d, train_labels = load_2d_dataset('TrainData/2D/')
        train_images_3d, _ = load_3d_dataset('TrainData/3D/')
        ...
        # Extract Hu-moments features for 2D images
        X_train_hu_2d = []
        for img in X_train_2d:
            moments = cv2.moments(img)
            hu_moments = cv2.HuMoments(moments).flatten()
            X_train_hu_2d.append(hu_moments)
        X_train_hu_2d = np.array(X_train_hu_2d)

        # Extract Hu-moments features for 3D images
        X_train_hu_3d = []
        for img in X_train_3d:
            moments = cv2.moments(img[:, :, 0])  # Use the first channel for Hu-moments
            hu_moments = cv2.HuMoments(moments).flatten()
            X_train_hu_3d.append(hu_moments)
        X_train_hu_3d = np.array(X_train_hu_3d)

        y_train = to_categorical(y_train)

        # Define the CNN model
        input_shape_2d = (7,)  # Hu-moments have 7 features
        input_shape_3d = (7,)  # Hu-moments have 7 features
        input_2d = Input(shape=input_shape_2d)
        input_3d = Input(shape=input_shape_3d)
        ...
        combined = Dropout(0.5)(combined)
        output = Dense(len(np.unique(train_labels)), activation='softmax')(combined)

        model = Model(inputs=[input_2d, input_3d], outputs=output)
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

        model.fit([X_train_hu_2d, X_train_hu_3d], y_train, epochs=10, batch_size=64)
        loss, accuracy = model.evaluate([X_train_hu_2d, X_train_hu_3d], y_train)
        print(f'Train accuracy: {accuracy}')
...
def test_model():
    ...
    if feature_var.get() == 'HOG' and classification_var.get() == 'CNN':
        ...
        # Reshape HOG features to fit the model input
        # hog_feature = hog_feature.reshape(1, 1764) Đưa cả đầu vào 2D và 3D
        hog_feature = hog_feature.reshape(1, 1764)

        # Load a dummy 3D input (since the model expects two inputs)
        ...
        # Reshape Hu-moments to fit the model input
